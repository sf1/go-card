...
type connectStruct struct {
    context uint32
    readerName [MAX_READERNAME]byte
    shareMode uint32
...
func PCSCLiteConnect() (*PCSCLiteClient, error) {
    ...
    err = client.ExchangeMessage(CMD_VERSION, ptr1[:])
...
func (client *PCSCLiteClient) SyncReaderStates() (
    ...
    err := client.SendHeader(CMD_GET_READERS_STATE, 0)
    _, err = client.Read(ptr[:])
    if err != nil { return count, err }
    for count = 0; count < PCSCLITE_MAX_READERS_CONTEXTS; count++ {
...
func (client *PCSCLiteClient) CardConnect(context uint32, readerName string) (
    ...
    if limit > MAX_READERNAME { limit = MAX_READERNAME }
    for i := 0; i < limit; i++ {
    ...
    err := client.ExchangeMessage(SCARD_CONNECT, ptr[:])
...
func (client *PCSCLiteClient) CardDisconnect(card int32) error {
    ...
    err := client.ExchangeMessage(SCARD_DISCONNECT, ptr[:])
    if err != nil { return err }
...
func (client *PCSCLiteClient) Transmit(card int32, protocol uint32,
    ...
    err := client.SendHeader(SCARD_TRANSMIT, uint32(len(tsBytes)))
    if err != nil { return 0, err }
...
func (client *PCSCLiteClient) WaitReaderStateChange() error {
    ...
    if wrstruct.rv == SCARD_E_TIMEOUT {
        client.ExchangeMessage(CMD_STOP_WAITING_READER_STATE_CHANGE, ptr[:])
        if err != nil { return err }
    }
